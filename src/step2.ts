...
import fs from "fs";
import { ProverX, ProverProofX } from './zkprogram.js';

export async function readProofFromFileAndVerify() {
    try {
        const { verificationKey: ProverX_VK } = await ProverX.compile({ forceRecompile: false });
        console.log('ProverX_VK: ' + ProverX_VK.hash);

        // read from file
        const str2 = fs.readFileSync('./proof.json', 'utf8');
        console.log('str2: ' + str2);
        const proof2 = await ProverProofX.fromJSON(JSON.parse(str2));
